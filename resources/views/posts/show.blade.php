...
<div class="container">
    ...
    <article class="blog-post mb-4">
        ...
        <div class="my-4 rounded-2">
            <img src="{{ url('storage/images/' . $post->image) }}" alt="Post Image" class="img-fluid rounded-3 shadow-lg" width="60%" height="auto">
        </div>
        @endif
        <p>{!! $post->content !!}</p>
    </article>
    ...
    <div class="my-4">
        ...
        <div class="comment pt-2">
            ...
            <div class="comment-container">
                <div>
                    <h6 class="nama_commentator pt-2">{{$comment->name}}<small class="px-2 text-muted fs-6 fw-light display-6">{{date('D H:i', strtotime($comment->created_at))}}</small></h6>
                    <p class="comment_commentator">{!!$comment->content!!}</p>
                </div>
                @if (Auth::check() && (Auth::user()->name == $comment->name || $post->author == Auth::user()->name))
