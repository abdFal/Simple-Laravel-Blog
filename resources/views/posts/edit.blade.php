    ...
    <div class="container">
        ...
        <form method="POST" action="{{ route("posts.update", $post->slug)}}" enctype="multipart/form-data">
            ...
            <button type="submit" name="back" class="btn btn-dark my-2">
                <a class="text-decoration-none text-light" href="{{ url('posts') }}">Back</a>
            </button>
            <button type="button" class="btn btn-danger px-4" data-bs-toggle="modal" data-bs-target="#exampleModal">Delete</button>
        </form>
    </div>
        ...
        <div class="modal-dialog modal-dialog-centered rounded-1">
            <div class="modal-content rounded-1">
                ...
                <div class="modal-body">
                    Apakah Anda yakin ingin menghapus postingan <span class="fw-bold">{{$post->title}}</span>
                </div>
                ...
                <button type="button" class="btn btn-primary" data-bs-dismiss="modal">Gajadi</button>
                </div>
            </div>
        </div>
    </div>
